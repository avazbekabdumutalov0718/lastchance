...
function renderTaskGrid(tasks) {
  ...
  tasks.forEach((task) => {
    ...
    card.innerHTML = `
      ...
      <button class="t-action" ${task.status === 'done' ? 'disabled' : ''}>${btnText}</button>
    `;
    card.querySelector('.t-action').addEventListener('click', () => handleTaskClick(task.id, task.status));
...
function tickTimers() {
  document.querySelectorAll('.task-card.in_progress').forEach((card) => {
    ...
    card.querySelector('.t-timer').textContent = fmtDuration(sec);
  });
}

async function handleTaskClick(id, status) {
  if (status === 'done') return;
  const action = status === 'in_progress' ? 'finish' : 'start';
...
async function loadMaterials(panel, section, kind) {
  ...
  list.innerHTML = rows.map((r) => `
    <div class="material-item">
      ...
      <div class="actions">
        <a href="/uploads/${r.filename}" target="_blank">Ochish</a>
